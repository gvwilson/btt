...
blank: True
title: "Security"
tag: "The most important part of software design"
syllabus:
- FIXME
---
...
measures to safeguard themselves against state-level
...
-   Intimate threats come from people who know their targets' passwords or have a
...
-   Insider threats come from people who abuse legitimate access to accounts and
...
1.  Have and enforce a Code of Conduct (FIXME) for any site you
...
1.  Keep a log of every administrative
    ...
    but if it is digitally signed there will still be evidence of tampering.
...
be compromised, in part because dictionary attacks can be run offline against stored passwords to
...
your fingerprints).  Two-factor authentication (2FA) requires two
...
port.  Sites like [Tech Solidarity][tech-solidarity]
...
software, or log into a website are called phishing attacks.  The strongest defense is to never do
...
spearphishing uses data harvested
...
why reporters and private investigators regularly go dumpster diving.) The best way to address this
...
depots in order to collect the raw material for attacks.)  Use a secure deletion tool like [BleachBit][bleachbit] (for
...
two needs: legislation like the EU's [General Data Protection Regulation][gdpr] (GDPR) includes a
...
> you.
>
> — [Heather Burns][burns-privacy-quote]
...
and with a few modifications, it can be used to exfiltrate data rather than delete it.

You defend against this by always sanitizing user input before using it. In this case, the query can
be written as:
...
A cross-site scripting attack works in the same way.
