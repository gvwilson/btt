...
As the name suggests, a unit test
...
-   A fixture, which is the thing being tested (e.g., an
...
-   An actual result, which is what the code produces when given the
    fixture.

-   An expected result that the actual result is
    compared to.

Good programmers often run informal unit tests interactively when debugging
(FIXME), but they are much more valuable when they can be re-run
at a moment's notice to make sure that the most recent changes haven't broken
anything that was working a few minutes ago.  To do this, you can use a test framework (also called a test runner).  Dozens of these have been built for almost every language you
...
If running all the tests is taking so long that it's disrupting your flow (FIXME), you can give `pytest` (and other
test runners) arguments to specify which subset of tests to run. This speeds up
development, but you should always re-run the entire test suite before committing your
...
sparked interest in fuzz testing
...
failing at about the same rate and for the same reasons.  FIXME
...
tested.  Conversely, if you design for test, it'll be a lot easier
to check whether your finished application actually does what it's supposed to.

Thinking about testability from the start turns out to be a good heuristic for design in general
...
For example, let's consider a typical three-tier web site that uses the Model-View-Controller
(MVC) design pattern. The model, which is stored in a relational database, is the data
...
are set, standard input and output are replaced by string I/O objects, and so on, so that the application has no (easy)
...
We can find that pretty easily with a CSS selector that looks for a `div` with the ID
...
that manages data on disk. An alternative is an embedded database, in which the
...
database is that it persists), but in a testing environment, an in-memory database can speed things up by a factor of thousands, since the hard
...
A third choice is to replace the database with a mock object.
...
in changes to the code. The downside is the loss of fidelity:
...
Most GUI testing frameworks have two other useful features. The first is record and playback: you can
...
same amount; in practice, the hot spots are usually
very easy to see.
</blockquote>
...
your code aren't being tested?  To find out, you can check the code coverage of your tests, i.e., measure the number of lines, statements, or
...
for 100% path coverage to ensure that every possible path through the
...
Some things are intrinsically harder to test than others. Take visualizations: any change to the dimension of the
plot, however small, can change many pixels in a raster
...
Computer graphics, data science, and anything else that uses floating-point arithmetic is also hard to test. One problem
