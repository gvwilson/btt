...
blank: True
title: "Code Review"
tag: "Sharing and reviewing work"
syllabus:
- FIXME
---

Version control really comes into
...
<!-- continue -->
And no, Git's
output here isn't particularly useful to anyone except people who are
...
PR.  When they click "Create Pull Request", Git gives it a unique serial number.  This is *not* a commit ID;
...
A [DuckDuckGo][duckduckgo] search for "how to write a good commit message" turns up several thousand articles. Most are
variations on the sample shown below; as with programming style
(FIXME), the most important thing is being consistent rather than the
...
exist is to allow code review.  One study after another since the mid-1970s has proven that code
...
:   If the person creating the PR goes through and adds some more comments,
    cleans up some variable names, and does a bit of refactoring (FIXME), they won't just make
...
:   The SmartBear guide recommends reviewing at most 400 lines at a time, which
    ...
    in their head at once ([%x cogload %]) and so will miss things.
...
How we respond to reviews is just as
important:
...
searches for duplicated files. FIXME table reference shows the
