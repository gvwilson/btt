...
tag: "The most neglected part of software design"
syllabus:
- FIXME
---
...
Most modern programming languages use exceptions for error handling.  In Python, for example,
...
exception).  We can catch an exception and inspect it as follows:
...
So where do exceptions come from?  The answer is that programmers can raise them
explicitly:
...
This behavior is designed to support a pattern called "throw low, catch high": write most of your code without exception
...
as early as possible so that we don't waste
...
defensive programming, and the
most common way to do it is to add assertions to our code so that it checks itself as it runs.
...
file isn't a CSV file makes it clear that the program only
...
is logging. Instead of writing `print`
...
levels in most logging libraries are `INFO`,
...
One of the recommendations in [%x security %] was to log actions to help you find suspicious activity.  When you do
...
Most logging libraries also support rotating files, i.e., they will write to `log.1` on the first day,
...
or YAML; you can easily write a small program to
